...
export default function manageGroups() {
  ...
  const [currentGroupIndexForAdd, setCurrentGroupIndexForAdd] =
    useState<number>(-1);
      ...
      if (groupsData && Array.isArray(groupsData)) {
        const mappedGroups = groupsData.map((g: any) => ({
          ...
          members: g.memberIds
            ? g.memberIds
                .map((mId: number) =>
                  players.find((p: any) => (p.userId || p.id) === mId),
                )
                .filter(Boolean)
            : g.members
              ? g.members
                  .map((m: any) =>
                    players.find(
                      (p: any) => (p.userId || p.id) === (m.userId || m.id),
                    ),
                  )
                  .filter(Boolean)
              : [],
  ...
  const removePlayerFromGroup = (groupIndex: number, memberIndex: number) => {
    ...
    if (nextGroups[groupIndex].scorerId === (player.userId || player.id)) {
      if (nextGroups[groupIndex].members.length > 0) {
        nextGroups[groupIndex].scorerId =
          nextGroups[groupIndex].members[0].userId ||
          nextGroups[groupIndex].members[0].id;
      } else {
    ...
    try {
      ...
      for (let i = 0; i < tournamentGroups.length; i++) {
        ...
        if (!g.scorerId) {
          Toast.show({
            type: "error",
            text1: `Group ${i + 1} is missing a scorer.`,
          });
          return;
        }
        if (g.members.length > 4) {
          Toast.show({
            type: "error",
            text1: `Group ${i + 1} exceeds max 4 players.`,
          });
          return;
        }
        payload.push({
                ...
                {group.members.map((member, memberIndex) => (
                  ...
                  >
                    <HStack style={{ alignItems: "center", flex: 1 }}>
                      <Pressable
                        onPress={() =>
                          setScorer(groupIndex, member.userId || member.id)
                        }
                        style={{ marginRight: 10 }}
                      >
                        <Ionicons
                          name={
                            group.scorerId === (member.userId || member.id)
                              ? "radio-button-on"
                              : "radio-button-off"
                          }
                          size={30}
                    ...
                    <Pressable
                      onPress={() =>
                        removePlayerFromGroup(groupIndex, memberIndex)
                      }
                    >
                      <Ionicons
                        name="close-circle"
