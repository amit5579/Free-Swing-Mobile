...
const PlayerCardSkeleton = ({ isExpanded = false }: { isExpanded?: boolean }) => {
  ...
  return (
    ...
    >
      <HStack className="items-center">
        <Skeleton isDark={isDark} width={45} height={45} borderRadius={24} style={{ marginRight: 12 }} />
        <VStack style={{ flex: 1 }}>
          ...
          <HStack style={{ flexWrap: "wrap", gap: 16 }}>
             {[1,2,3,4].map(i => (
                <VStack key={i} style={{ width: "47%" }}>
                  ...
                  <Skeleton isDark={isDark} width={40} height={15} />
                </VStack>
             ))}
          </HStack>
          <HStack style={{ marginTop: 24, justifyContent: "flex-end" }}>
  ...
  if (loading) {
    return (
      <View style={{ flex: 1, backgroundColor: isDark ? "#000" : "#f2f2f2", paddingHorizontal: 16, paddingTop: 10 }}>
        {[1, 2, 3, 4, 5, 6].map((i) => (
          <PlayerCardSkeleton key={i} isExpanded={i === 1} />
        ))}
      </View>
