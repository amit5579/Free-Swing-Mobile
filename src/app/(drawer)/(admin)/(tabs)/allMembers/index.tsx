...
export default function AllMembersScreen({
  ...
  const filteredMembers = members.filter((member) => {
    ...
    if (activeTab === "Blocked") return matchesSearch && member.isBlocked && !member.hasPendingSubscriptionRequest;
  ...
  return (
    ...
    >
      ...
      <VStack style={{ paddingTop: 10 }}>
        ...
        {hideAdminControls && (
          <HStack className="items-center justify-between px-5 mb-4" style={{ zIndex: 100 }}>
            <ThemedText style={{ fontSize: 22, fontWeight: "800", letterSpacing: -0.5 }}>
              Members
            </ThemedText>
            <Box style={{ position: "relative" }}>
                <Box style={{ backgroundColor: "#8BC34A", paddingHorizontal: 14, paddingVertical: 6, borderRadius: 12, shadowColor: "#8BC34A", shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.3, shadowRadius: 6, elevation: 4 }}>
                <ThemedText style={{ fontSize: 12, fontWeight: "900", color: "#111" }}>
                    Total: {members.length}
                </ThemedText>
                </Box>
                {members.filter(m => m.hasPendingSubscriptionRequest).length > 0 && (
                    <Box 
                        style={{ 
                            position: 'absolute', 
                            top: -6, 
                ...
                {tabs.map((tab) => {
                    ...
                    >
                        ...
                        style={{
                            ...
                            color: isActive ? "#111" : isDark ? "#94a3b8" : "#64748b",
                        }}
                        >
                        {tab}
                        </ThemedText>
      ...
      {loading ? (
        ...
        >
          <VStack>
            <MemberCardSkeleton isDark={isDark} isExpanded={!hideAdminControls} hideAdminControls={hideAdminControls} />
            <MemberCardSkeleton isDark={isDark} hideAdminControls={hideAdminControls} />
            <MemberCardSkeleton isDark={isDark} hideAdminControls={hideAdminControls} />
            <MemberCardSkeleton isDark={isDark} hideAdminControls={hideAdminControls} />
          </VStack>
        </ScrollView>
                        ...
                        >
                            ...
                            <HStack className="items-center" space="md">
                                <Box style={{ position: "relative" }}>
                                    ...
                                    >
                                        {member.profilePictureUrl && !imageErrors[member.id] ? (
                                            <AvatarImage 
                                                source={{ uri: member.profilePictureUrl.startsWith('http') ? member.profilePictureUrl : `https://kolve18freeswing.com${member.profilePictureUrl}` }} 
                                ...
                                <VStack style={{ gap: -1 }}>
                                    <ThemedText style={{ fontSize: 18, fontWeight: "900" }} numberOfLines={1}>
                                        {member.username}
                                    </ThemedText>
                                    <HStack space="xs" className="items-center" style={{ opacity: 0.5 }}>
                                        <Ionicons name="person-outline" size={10} color={isDark ? "#fff" : "#111"} />
                                        <ThemedText style={{ fontSize: 9.5, textTransform: "uppercase", fontWeight: "700", letterSpacing: 0.5 }}>
                                            {member.role || "Member"}
                                        </ThemedText>
                                    </HStack>
                                </VStack>
                            ...
                            <View style={{ height: 1, backgroundColor: isDark ? "rgba(255,255,255,0.05)" : "rgba(0,0,0,0.05)", marginVertical: 16 }} />

                            <HStack className="justify-between items-center" style={{ marginBottom: 12 }}>
                                <VStack style={{ flex: 1 }}>
                                    <ThemedText style={{ fontSize: 11, opacity: 0.6, fontWeight: '600' }}>HOME COURSE</ThemedText>
                                    <ThemedText style={{ fontSize: 13, fontWeight: '700' }}>{member.homeCourse || "Not added yet"}</ThemedText>
                                </VStack>
                                <VStack style={{ alignItems: 'flex-end' }}>
                                    <ThemedText style={{ fontSize: 11, opacity: 0.6, fontWeight: '600' }}>HC INDEX</ThemedText>
                                    <ThemedText style={{ fontSize: 13, fontWeight: '800', color: "#84cc16" }}>{member.handicapIndex ?? "NA"}</ThemedText>
                                </VStack>
                            </HStack>

                            <HStack className="justify-between items-center">
                                <VStack style={{ flex: 1 }}>
                                    <ThemedText style={{ fontSize: 11, opacity: 0.6, fontWeight: '600' }}>ADDED BY</ThemedText>
                                    <ThemedText style={{ fontSize: 13, fontWeight: '700' }}>{member.invitedBySubAdminName || "Direct / Admin"}</ThemedText>
                                </VStack>
                                <TouchableOpacity 
                                    onPress={async () => {
                                        const role = await AsyncStorage.getItem("role");
                                        if (role?.toLowerCase() === 'admin' || role?.toLowerCase() === 'subadmin') {
                                            router.push(`/(drawer)/(admin)/(tabs)/allMembers/${member.id}`);
                                        } else {
                                            router.push(`/(drawer)/(user)/(tabs)/dashboard/tabs/${member.id}`);
                                        }
                                    }}
                                    style={{ 
                                        backgroundColor: "#8BC34A", 
                                ...
                                >
                                    <ThemedText style={{ color: 'white', fontWeight: '900', fontSize: 12 }}>View Profile</ThemedText>
                                </TouchableOpacity>
                            </HStack>
                          </View>
                        </Box>
                  ...
                  return (
                    <Box
                      ...
                      style={{
                        ...
                        marginBottom: 16,
                        overflow: "hidden",
                        shadowColor: member?.hasPendingSubscriptionRequest ? "transparent" : member.isBlocked ? "#DC2626" : "#8BC34A",
                        shadowOffset: { width: 0, height: 6 },
                        shadowOpacity: member?.hasPendingSubscriptionRequest ? 0 : (isDark ? 0.3 : 0.1),
                        shadowRadius: 12,
                        elevation: member?.hasPendingSubscriptionRequest ? 0 : 3,
                      }}
                    >
                    <BlurView
                            ...
                            <HStack className="items-center" style={{ gap: 8 }}>
                              ...
                              <Box
                                style={{
                                  ...
                                  borderRadius: 8,
                                  borderWidth: 1,
                                  borderColor: `${member?.hasPendingSubscriptionRequest
                                      ? "#F59E0B"
                                      : member.isBlocked
                                        ? "#DC2626"
                                        : "#8BC34A"
