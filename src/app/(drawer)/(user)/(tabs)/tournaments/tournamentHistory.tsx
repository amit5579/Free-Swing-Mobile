...
const TournamentHistory: React.FC = () => {
  ...
  const currentScorecardIdRef = useRef<number | string | null>(
    scorecardId ? Number(scorecardId) : null,
  );
  ...
  useEffect(() => {
    ...
    interval = setInterval(async () => {
      ...
      const activeId =
        currentScorecardIdRef.current ||
        scorecardId ||
        (holesRef.current[0] as any)?.scorecardId ||
        tournamentId;

      if (activeId) {
        try {
          const liveData = await getScorecardDetails(Number(activeId));
          ...
          console.error("Polling live tournament scorecard error:", e);
        }
      }
    }, pollIntervalTime);

    return () => clearInterval(interval);
  }, [scorecardId, tournamentId, roundContextId, isReadOnly, isCompanionView]);
  ...
  const handleBack = useCallback(async () => {
    ...
    if (holesRef.current && holesRef.current.length > 0 && activeId) {
      ...
      const payload = holesRef.current.map((h: any) => ({
        ...
        tournamentId: h.tournamentId || Number(tournamentId) || null,
        holeId: h.holeId,
      ...
      try {
        await updateHoleScoresApi(activeId, payload);
      } catch (err) {
        console.error("Final tournament save failed:", err);
      }
    }
  ...
  useEffect(() => {
    const fetchScorecard = async () => {
      try {
        ...
        if (!finalScorecardId && tournamentId) {
          ...
          } = require("@/api/modules/admin/tournaments.api");
        ...
        const data = await getScorecardDetails(finalScorecardId);
        ...
        if (data.length > 0) {
          // console.log("HOLE KEYS:", Object.keys(data[0]));
          // console.log("PARTNERS JSON:", (data[0] as any).playingPartnersJson);
          // console.log("COMPANION SCORES JSON:", (data[0] as any).companionScoresJson);
        }
    ...
    fetchScorecard();
  }, [scorecardId, tournamentId, displayHandicap, getDisplayHandicap]);
  ...
  const saveToServer = async (holesToSave: ScorecardHole[]) => {
    const activeId =
      currentScorecardIdRef.current ||
      scorecardId ||
      (holesToSave[0] as any)?.scorecardId ||
      tournamentId;
    if (!activeId) return;
    ...
    const performSave = async () => {
      try {
        ...
        console.error("Sync failed for tournament scorecard:", activeId, err);
        return false;
      }
    };
  ...
  const handleSave = async () => {
    try {
      ...
      await updateScorecardApi(scorecardId!, holeScores);
    } catch (err) {
      console.error(err);
    } finally {
      setSaving(false);
      ...
      { text: "Cancel", style: "cancel" },
      {
        ...
        onPress: async () => {
          try {
            ...
            await saveScorecardApi(scorecardId!);
